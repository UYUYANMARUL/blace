...
use alloy::rlp::{Buf, BufMut, Encodable, Header, EMPTY_STRING_CODE};
use futures_lite::StreamExt;
use hashbrown::{HashMap, HashSet};
use keccak_hash::{keccak, KECCAK_NULL_RLP};
use tokio::sync::RwLock;

use crate::mpt::db::{AsyncDB, MemoryDB};
use crate::mpt::errors::TrieError;
use crate::mpt::nibbles::Nibbles;
use crate::mpt::node::{empty_children, BranchNode, Node};
...
pub trait Trie<D: AsyncDB> {
    /// Returns the value for key stored in the trie.
    async fn get(&self, key: &[u8]) -> TrieResult<Option<Vec<u8>>>;

    /// Checks that the key is present in the trie
    async fn contains(&self, key: &[u8]) -> TrieResult<bool>;

    /// Inserts value into trie and modifies it if it exists
    async fn insert(&mut self, key: &[u8], value: &[u8]) -> TrieResult<()>;
    ...
    async fn root_hash(&mut self) -> TrieResult<B256>;

    /// Saves all the nodes in the db, clears the cache data, recalculates the root.
    /// Returns the root hash of the trie and updated nodes from the cache.
    async fn root_hash_with_changed_nodes(&mut self) -> TrieResult<RootWithTrieDiff>;

    /// Clears the whole trie from the database.
    async fn clear_trie_from_db(&mut self) -> TrieResult<()>;
    ...
    async fn get_proof(&mut self, key: &[u8]) -> TrieResult<Vec<Vec<u8>>>;
    ...
    D: AsyncDB,
{
    ...
    pub async fn from(db: Arc<D>, root: B256) -> TrieResult<Self> {
        match db
            .get(root.as_slice())
            .await
    ...
    D: AsyncDB,
{
    ...
    async fn get(&self, key: &[u8]) -> TrieResult<Option<Vec<u8>>> {
        ...
        let result = self.get_at(&self.root, path, 0).await;
    ...
    async fn insert(&mut self, key: &[u8], value: &[u8]) -> TrieResult<()> {
        ...
        let path = &Nibbles::from_raw(key, true);
        let result = self.insert_at(root, path, 0, value.to_vec()).await;
    ...
    async fn remove(&mut self, key: &[u8]) -> TrieResult<bool> {
        let path = &Nibbles::from_raw(key, true);
        let result = self.delete_at(&self.root.clone(), path, 0).await;
    ...
    async fn get_proof(&mut self, key: &[u8]) -> TrieResult<Vec<Vec<u8>>> {
        ...
        let result = self.get_path_at(&self.root, key_path, 0).await;
        ...
        }) = result
        {
            ...
            for path in path.into_iter().rev() {
                let encoded = self.encode_raw(&path).await;
                return_path.push(encoded);
            }
    ...
    async fn verify_proof(
        ...
        let trie = EthTrie::from(proof_db, root_hash)
            .await
            .or(Err(TrieError::InvalidProof))?;
        trie.get(key).await.or(Err(TrieError::InvalidProof))
    }
}
    ...
    D: AsyncDB,
{
    async fn get_at(
        ...
        match source_node {
            ...
            Node::Branch(branch) => {
                let borrow_branch = branch.read().await;

                if partial.is_empty() || partial.at(0) == 16 {
                    Ok(borrow_branch.value.clone())
                } else {
                    let index = partial.at(0);
                    Box::pin(self.get_at(&borrow_branch.children[index], path, path_index + 1))
                        .await
                }
            }
            Node::Extension(extension) => {
                ...
                if match_len == prefix.len() {
                    Box::pin(self.get_at(&extension.node, path, path_index + match_len)).await
                } else {
                    Ok(None)
                }
            }
            Node::Hash(hash_node) => {
                ...
                let node = self.recover_from_db(node_hash).await?.ok_or_else(|| {
                ...
                Box::pin(self.get_at(&node, path, path_index)).await
            }
        }
    }
    ...
    async fn insert_at(
        ...
        match n {
            ...
            Node::Branch(branch) => {
                let mut borrow_branch = branch.write().await;
                ...
                let new_child =
                    Box::pin(self.insert_at(child, path, path_index + 1, value)).await?;
                borrow_branch.children[partial.at(0)] = new_child;
                Ok(Node::Branch(branch.clone()))
            }
            Node::Extension(ext) => {
                let mut borrow_ext = ext.write().await;
                ...
                if match_index == 0 {
                    ...
                    return Box::pin(self.insert_at(node, path, path_index, value)).await;
                }

                if match_index == prefix.len() {
                    let new_node =
                        Box::pin(self.insert_at(sub_node, path, path_index + match_index, value))
                            .await?;
                ...
                let new_node =
                    Box::pin(self.insert_at(new_ext, path, path_index + match_index, value))
                        .await?;
            ...
            Node::Hash(hash_node) => {
                ...
                Box::pin(self.insert_at(node, path, path_index, value)).await
            }
        }
    }
        ...
        let (new_node, deleted) = match old_node {
            ...
            Node::Branch(branch) => {
                ...
                let (new_child, deleted) =
                    Box::pin(self.delete_at(child, path, path_index + 1)).await?;
                if deleted {
            ...
            Node::Extension(ext) => {
                ...
                if match_len == prefix.len() {
                    let (new_node, deleted) =
                        Box::pin(self.delete_at(&borrow_ext.node, path, path_index + match_len))
                            .await?;
            ...
            Node::Hash(hash_node) => {
                ...
                let node = self.recover_from_db(hash).await?.ok_or_else(|| {
                    TrieError::MissingTrieNode {
                        node_hash: hash,
                        traversed: Some(path.slice(0, path_index)),
                        root_hash: Some(self.root_hash),
                ...
                Box::pin(self.delete_at(&node, path, path_index)).await
            }
        }?;

        if deleted {
            Ok((self.degenerate(new_node).await?, deleted))
        } else {
            Ok((new_node, deleted))
        }
    }
    ...
    async fn degenerate(&mut self, n: Node) -> TrieResult<Node> {
        match n {
            Node::Branch(branch) => {
                let borrow_branch = branch.read().await;
            ...
            Node::Extension(ext) => {
                ...
                match borrow_ext.node.clone() {
                    ...
                    Node::Hash(hash_node) => {
                        ...
                        let new_node = self.recover_from_db(node_hash).await?.ok_or(
                            TrieError::MissingTrieNode {
                                node_hash,
                                traversed: None,
                                root_hash: Some(self.root_hash),
                        ...
                        Box::pin(self.degenerate(n)).await
                    }
        ...
        match source_node {
            ...
            Node::Branch(branch) => {
                ...
                if partial.is_empty() || partial.at(0) == 16 {
                    ...
                    Box::pin(self.get_path_at(node, path, path_index + 1)).await
                }
            }
            Node::Extension(ext) => {
                let borrow_ext = ext.read().await;
                ...
                if match_len == prefix.len() {
                    Box::pin(self.get_path_at(&borrow_ext.node, path, path_index + match_len)).await
                } else {
                    Ok(vec![])
                }
            }
            Node::Hash(hash_node) => {
                ...
                let mut rest = Box::pin(self.get_path_at(&n, path, path_index)).await?;
                rest.push(n);
                Ok(rest)
            }
        }
    ...
    async fn commit(&mut self, return_changed_nodes: bool) -> TrieResult<RootWithTrieDiff> {
        let root_hash = match self.write_node(&self.root.clone()).await {
    ...
    async fn write_node(&mut self, to_encode: &Node) -> EncodedNode {
        ...
        let data = Box::pin(self.encode_raw(to_encode)).await;
    ...
    async fn encode_raw(&mut self, node: &Node) -> Vec<u8> {
        match node {
            ...
            Node::Extension(ext) => {
                ...
                borrow_ext
                    ...
                    .as_slice()
                    .encode(&mut list);
                match self.write_node(&borrow_ext.node).await {
                    EncodedNode::Hash(hash) => hash.as_slice().encode(&mut list),
                    EncodedNode::Inline(data) => list.extend_from_slice(data.as_slice()),
                };
                let header = Header {
    ...
    async fn recover_from_db(&self, key: B256) -> TrieResult<Option<Node>> {
        let node = match self
            .db
            .get(key.as_slice())
            .await
...
//     async fn test_trie_insert_corrupt() {
//         let (mut trie, actual_root_hash, deleted_node_hash) = corrupt_trie();
//
...
//
//     #[tokio::test]
//     async fn test_large_trie_at_root() {
//         let memdb = Arc::new(MemoryDB::new(true));
//         let mut trie = EthTrie::new(memdb.clone());
//         trie.insert(
//             b"pretty-long-key",
...
//         .await
//         .unwrap();
//         let new_root_hash = trie.root_hash().await.unwrap();
//
//         let empty_trie = EthTrie::new(memdb.clone());
//         // Can't find key in new trie at empty root
